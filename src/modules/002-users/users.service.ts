...
export class UserService {
  ...
  getUserById = async (req: Request, res: Response): Promise<Response> => {
    ...
    const isFriend = (user.friends as any[])?.some((f) =>
  ...
  uploadCoverImage = async (req: Request, res: Response): Promise<Response> => {
    const { _id } = req.tokenDecoded as JwtPayload;
  ...
  deleteProfilePicture = async (
    ...
    const { _id } = req.tokenDecoded as JwtPayload;
    const public_id = req.user?.picture?.public_id;

    if (!public_id) {
      throw new NotFoundException("User Has No Profile Picture");
    }

    const deleted = await deleteImageFromCloudinary(public_id);
  ...
  deleteCoverImages = async (
    ...
    const { _id } = req.tokenDecoded as JwtPayload;
    const public_id = req.user?.coverImage?.public_id;

    if (!public_id) {
      throw new NotFoundException("User Has No Cover Image");
    }
  ...
  sendFriendRequest = async (
    ...
    if (
      await this.friendRequestModel.findOne({
        filter: {
  ...
  cancelFriendRequest = async (
    ...
    const Request = await this.friendRequestModel.findOneAndDelete({
      filter: {
        _id: requestId,
        acceptedAt: { $exists: false },
        $or: [{ sendBy: req.user?._id }, { sendTo: req.user?._id }],
      },
    });
    ...
    const removeFriend = await Promise.all([
      ...
      this.friendRequestModel.findOneAndDelete({
        filter: {
          ...
          $and: [
            {
              $or: [{ sendBy: req.user?._id }, { sendBy: userId }],
            },
            {
              $or: [{ sendTo: req.user?._id }, { sendTo: userId }],
            },
          ],
        },
      }),
  ...
  updateBasicInfo = async (req: Request, res: Response): Promise<Response> => {
    ...
    let data: IUpdateData = {
      ...
      slug: req.body.validData.userName
        .replaceAll(/\s+/g, "-")
        .toLocaleLowerCase(),
  ...
  unfreezeAccountByAccountAuthor = async (
    req: Request,
    res: Response
  ): Promise<Response> => {
